...
setup_multinodes_cometbft() {
  for container in "${containers[@]}"; do
    ...
    docker cp "$HOME/cometbftconfig/go.sum" "$container":/root/ || { echo "Failed to copy go sum file to $container"; exit 1; }
    docker cp "$HOME/cometbftconfig/main.go" "$container":/root/ || { echo "Failed to copy abci main go file to $container"; exit 1; }
    docker exec "$container" bash -c "cd /root && /usr/local/go/bin/go clean -modcache && /usr/local/go/bin/go mod tidy && /usr/local/go/bin/go build -o /root/abci-app main.go"
    ...
    docker exec "$container" /opt/serfapp/serf tags -set rpc_addr="$nodeId@$ip_address:7373"
