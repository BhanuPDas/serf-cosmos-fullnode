...
def dial_peers(rpc_peers: list[str], mempool_client):
    try:
        mempool_client.dial_peers(rpc_peers, persistent=True)
    except Exception as e:
        logger.error(f"Error while dialing peers: {e}")
...
def serf_monitor_thread():
    logger.info(f"Starting Serf monitor thread.")
    ...
    while True:
        ...
        if current_time - last_members_check_time > MEMBER_CHECK_INTERVAL:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info("Dialing peers...")
                    dial_peers(rpc_peers, cometbft)
                else:
                    logger.error(f"Failed to get Serf members: {response.status_code}")
                    with metrics_lock:
                        app_metrics["serf_rpc_status"] = "Disconnected"
                        app_metrics["serf_monitor_status"] = "Failed to get members"
                        app_metrics["serf_monitor_last_error"] = "No Response from Serf" if response.status_code != 200 else None
                last_members_check_time = current_time
            except Exception as e:
                logger.error(f"Error fetching Serf members: {e}")
                with metrics_lock:
                    app_metrics["serf_rpc_status"] = "Error"
                    app_metrics["serf_monitor_last_error"] = f"Members fetch error: {e}" if e else None
                last_members_check_time = current_time

        # Periodically check CometBFT RPC status
        if current_time - last_cometbft_status_check_time > COMETBFT_STATUS_CHECK_INTERVAL:
            try:
                comet_status_data = cometbft.get_status()
                with metrics_lock:
        ...
        try:
                    ...
                    for msg_id, data in messages:
                        ...
                        try:
                            ...
                            if event_name.startswith("transfer"):
                                ...
                                process_serf_user_event(event_name, payload_b64, cometbft)
                                ...
                                if success:
                                    if res:
                                        ...
                                        logger.info(f"Publishing Polling Results....")
                                        r.publish(channel, json.dumps(tx_result))
                                else:
                                    logger.info(msg)
            ...
            with metrics_lock:
                ...
                app_metrics["serf_monitor_last_error"] = f"Startup error: {e}" if e else None
            time.sleep(5)
